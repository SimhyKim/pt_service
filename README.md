...
TF-IDF + MiniBatch K-Means были выбраны как быстрое решение, которое гарантирует приближенность кластеризации логов к реальному времени (не будет большой задержки между поступлением лога на сервис и дальнейшей его отправкой в обогащатор/logstash)
